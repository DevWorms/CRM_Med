                ...
                if (response.pacientes.length > 0) { 
                    ...
                    response.pacientes.forEach(function (p) { 
                        ...
                        $("#pacientes_div").append( 
                            '<h5 id="paciente_' + i + '" style="cursor:pointer"> ' + i + '. ' + p.id + ' - ' + nombre + ' </h5>' + 

                            '<a class="btn btn-info btn-sm" onclick="showDetail(' + p.id + ')" >Ver resumen de paciente</a> ' + 
                            '<a class="btn btn-success btn-sm" href="' + APP_URL + 'expediente_folio#' + p.id + '" >Ver expediente completo</a> <br>' + 
        ...
        success: function (response) { 
            ...
            if (response.estado == 1) { 
                response.procedimientos.forEach(function (proc) { 
                    ...
                    var table = document.getElementById('tratamientos_' + id_paciente);
